...
def isbnChecker(isbn):
    ...
    for num in range(0, 13):
        # every even num is *1 then added to total
        if num % 2 == 0 or num == 0:
...
def addBook():
    ...
    while placeholder2 is True:
        isbn = (input("ISBN> "))
        # If isbn has letters it is not valid
        if not isbn.isdigit():
            print("Invalid ISBN!")
    ...
    # Finally adds all the gathered information into the allBooks list if isbn is valid
    if placeholder4 is False:
